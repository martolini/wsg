...
class App extends Component {
  ...
  getOptions = (input) => {
    ...
    return axios.get(`http://wsgapi.msroed.io/search`, 
      {
        params: {
          s: input,
          type: 'series'
  ...
  getDetails = ({value}) => {
    this.setState({loading: true})
    axios.get(`http://wsgapi.msroed.io/get/${value}`)
      .then((response) => {
        this.setState({
  ...
  showRatingsfilteredByRange = () => {
    return (this.state.selectedShowRatings || []).filter(show => show.imdbRating > this.state.range[0] && show.imdbRating <= this.state.range[1])
  }
    ...
    return (
      <div className="App">
        ...
        <footer className="footer">
          <p>Using the <a target="_blank" rel="noopener noreferrer" href="http://www.omdbapi.com/">OMDb API</a>.
          </p>
        </footer>
        <EpisodeModal 
